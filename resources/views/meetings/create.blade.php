@extends('layouts.app')
@section('title', isset($meeting) ? 'Edit Meeting Detail' : 'Add Meeting Date')
    ...
    <div class="col-md-7">
        <div class="d-flex align-items-center gap-3 mb-4">
            ...
            <h5 class="mb-0 fw-bold border-start ps-3" style="border-width: 3px !important; border-color: var(--primary) !important;">
                {{ isset($meeting) ? 'Edit Meeting Detail' : 'Add Meeting Date' }}
            </h5>
        </div>
            ...
            <div class="card-body p-5">
                <form action="{{ isset($meeting) ? route('meetings.update', $meeting) : route('meetings.store') }}" method="POST">
                    ...
                    <div class="row g-4 mb-5">
                        <div class="col-md-12">
                            <label class="form-label small fw-bold text-uppercase text-muted ls-wide">Meeting Date <span class="text-danger">*</span></label>
                            <input type="date" name="meeting_date" class="form-control form-control-lg bg-light border-0 @error('meeting_date') is-invalid @enderror" 
                                value="{{ old('meeting_date', isset($meeting->meeting_date) ? $meeting->meeting_date->format('Y-m-d') : '') }}" required>
                            @error('meeting_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            <div class="form-text small opacity-75">The month and year for tracking will be set automatically from this date.</div>
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-3">
                        ...
                        <button type="submit" class="btn btn-lg btn-primary px-5 rounded-3 shadow" style="background: #1e3a5f;">
                            <i class="bi bi-calendar-check me-2"></i>{{ isset($meeting) ? 'Update Meeting' : 'Add to Calendar' }}
                        </button>
                    </div>
                </form>
            </div>
